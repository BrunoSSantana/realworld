...
import { AddCommentApi, ApiCreateArticle, ApiUserCreate, ApiUserLogin } from '@/ports/fastify/types'
...
app.post<AddCommentApi>('/api/articles/:slug/comments', authOptions, async (req, reply) => {
  const payload = req.raw.auth
  const idProp = 'id'
  const slugProp = 'slug'
  ...
  return pipe(
    ...
    TE.map(commentToAnArticle => reply.status(201).send(commentToAnArticle)),
    TE.mapLeft(err => reply.status(422).send(err)),
  )()
})
